export function Card({ children, className = '' }) {
  return (
    <section className={`rounded-xl border border-line bg-white shadow-soft ring-1 ring-black/[0.01] ${className}`}>
      {children}
    </section>
  );
}
...
export function DashboardCard({ title, value, icon: Icon, footer, className = '' }) {
  return (
    <Card className={`p-5 transition duration-150 hover:-translate-y-0.5 hover:shadow-md ${className}`}>
